...
class ScanEngine:
    ...
    def _load_signatures() -> dict[str, Any]:
        sig_paths = [
            Path(__file__).parent.parent / "signatures" / "ai_signatures.yaml",  # core/signatures/
            Path("/config") / "ai_signatures.yaml",  # Docker mount
            Path(__file__).parent.parent.parent / "config" / "ai_signatures.yaml",  # repo root config/
        ]
        for sig_path in sig_paths:
            if sig_path.exists():
                try:
                    ...
                    logger.warning("Failed to load signatures from %s", sig_path, exc_info=True)
