      ...
      try {
        ...
        Object.entries(changesByFile).forEach(([fileName, fileChanges]) => {
          ...
          try {
            ...
            execSync(`git -c user.name="visual-edit" -c user.email="noreply@scanllm.ai" add "${targetFile}"`);
            execSync(`git -c user.name="visual-edit" -c user.email="noreply@scanllm.ai" commit -m "visual_edit_${timestamp}"`);
          } catch (gitError) {
